...
public class CommandFramework implements CommandExecutor {
        ...
        for (int i = args.length; i >= 0; i--) {
            ...
            if (commandMap.containsKey(cmdLabel)) {
                ...
                if (!command.permission().equals("") && (!sender.hasPermission(command.permission()))) {
                    sender.sendMessage(ZeroLib.INSTANCE.getNoPermissionMessage()); // TODO change
                    return true;
                }
                if (command.inGameOnly() && !(sender instanceof Player)) {
                    sender.sendMessage(ZeroLib.INSTANCE.getOnlyGameCommandMessage());
                    return true;
                }
    ...
    public void loadCommandsInFile() {
        ...
        if (ZeroLib.INSTANCE.isExcludeCommands()) {
            config.set("EXCLUDE-COMMANDS", ZeroLib.INSTANCE.getExcludeCommandsList());
        }
        config.set("TOTAL-REGISTER-COMMANDS", config.getKeys(false).size());
        try {
            config.save(file.getFile());
        } catch (IOException e) {
            ZeroLib.INSTANCE.getPlugin().getLogger().warning("[Commands] Error in register commands.yml");
        }
    }
    ...
    public void registerCommands(Object obj, List<String> aliases) {
        for (Method m : obj.getClass().getMethods()) {
            if (m.getAnnotation(Command.class) != null) {
                ...
                boolean parse = false;
                if (ZeroLib.INSTANCE.isExcludeCommands()) {
                    if (ZeroLib.INSTANCE.getExcludeCommandFile() != null && ZeroLib.INSTANCE.getExcludeCommandsPath() != null) {
                        for (String disabled : ZeroLib.INSTANCE.getExcludeCommandsList()) {
                ...
                if (parse) {
                    Bukkit.getConsoleSender().sendMessage(ZeroLib.INSTANCE.getDisableCommandMessage().replace("<command>", command.name()));
                    continue;
                }
