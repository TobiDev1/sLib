...
public class ButtonListener implements Listener {
      ...
      if (openMenu != null) {
         ...
         if (openMenu.getButtons().containsKey(event.getSlot())) {
            ...
            if (Menu.currentlyOpenedMenus.containsKey(player.getName())) {
               ...
               if (newMenu == openMenu) {
                  button.shouldUpdate(player, event.getSlot(), event.getClick());
            ...
            if (event.isCancelled()) {
               Bukkit.getScheduler().runTaskLater(ZeroLib.INSTANCE.getPlugin(), () -> {
   ...
   public void onInventoryClose(InventoryCloseEvent event) {
      ...
      player.setMetadata("scanglitch", new FixedMetadataValue(ZeroLib.INSTANCE.getPlugin(), true));
   }
   ...
   public void onPlayerMove(PlayerMoveEvent event) {
      ...
      if (player.hasMetadata("scanglitch")) {
         player.removeMetadata("scanglitch", ZeroLib.INSTANCE.getPlugin());
         ItemStack[] var3 = player.getInventory().getContents();
         int var4 = var3.length;

         for(int var5 = 0; var5 < var4; ++var5) {
            ItemStack it = var3[var5];
            if (it != null) {
