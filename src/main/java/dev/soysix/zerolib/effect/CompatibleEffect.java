package dev.soysix.zerolib.effect;

import dev.soysix.zerolib.utils.ServerUtil;
...
public enum CompatibleEffect {
    ...
    private Effect getEffect() {
        if (ServerUtil.SERVER_VERSION_INT == 8) {
            return effect8 == null ? Effect.valueOf("HEART") : Effect.valueOf(effect8);
        } else if (ServerUtil.SERVER_VERSION_INT > 9 && ServerUtil.SERVER_VERSION_INT < 13) {
            return effect912 == null ? Effect.valueOf("HEART_PARTICLE") : Effect.valueOf(effect912);
        } else if (ServerUtil.SERVER_VERSION_INT >= 13) {
